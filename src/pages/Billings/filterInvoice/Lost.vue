<template>
  <div>
    ...
    <!-- <div class="font-medium text-3xl text-900 font-bold text-blue-900 mb-3">
      Hello, {{userName}}
    </div> -->
       <div class="card">
      <TabMenu :model="items" />
...
export default {
  ...
  methods: {
    async fetchCummions() {
      ...
      const q = query(collection(db, 'invoices'), where('invoiceInfo.status', '==', 'Lost'), where('userId','==',this.uid));
